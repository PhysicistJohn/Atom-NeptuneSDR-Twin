...
canonical [P210 FFT ABI](https://github.com/PhysicistJohn/Atom-NeptuneSDR_Firmwave/blob/main/docs/P210_FFT_ABI.md).
Neither repository imports the other's implementation; they meet at Firmwave's
versioned JSON interface and generated runtime manifest.
The `Firmwave` spelling in the repository and directory name is intentional.

The Twin pins the companion repository by URL, full commit, tree, and interface
SHA-256 in `deps/firmwave.lock.json`. The resolver
uses `../Atom-NeptuneSDR_Firmwave` when that exact clean checkout matches the
lock. Set `NEPTUNESDR_FIRMWAVE_ROOT` for another location. If neither exists,
the resolver clones only the pinned commit into the Twin-owned
`.cache/deps/firmwave/`; it never updates, checks out, cleans, or otherwise
mutates a user-managed checkout. Inspect the decision with:

```sh
python3 scripts/resolve_firmwave.py --json
```
...
QEMU 10.0.2 machine, asks the locked Firmwave checkout to build the ARM FFT
...
manifest bound to both exact Git source states, the Firmwave interface and
...
[canonical Firmwave FFT register/DMA ABI](https://github.com/PhysicistJohn/Atom-NeptuneSDR_Firmwave/blob/main/docs/P210_FFT_ABI.md).
...
The same gate resolves and tests the exact Firmwave revision as described
...
repository assets. The ARM service and firmware composition are Firmwave
...
Firmware and PL handoff inputs are owned by Firmwave, content-addressed, and
inspected without a board. Resolve the locked checkout and invoke its CLI;
these commands never flash a board:

```sh
FIRMWAVE_ROOT=$(python3 scripts/resolve_firmwave.py)
FIRMWAVE_CACHE="$FIRMWAVE_ROOT/.cache/firmware"
export PYTHONPATH="$FIRMWAVE_ROOT/src${PYTHONPATH:+:$PYTHONPATH}"
python3 -m neptunesdr_firmwave validate-locks
python3 -m neptunesdr_firmwave fetch \
  p210-sd-boot p210-system-xsa plutosdr-fw-v0.39 \
  --cache-dir "$FIRMWAVE_CACHE"
python3 -m neptunesdr_firmwave validate-xsa \
  --artifact p210-system-xsa --cache-dir "$FIRMWAVE_CACHE"
```

Firmwave can assemble the complete non-flashable QEMU-development bundle or
run its narrower bounded boot harness:

```sh
ZIG=/path/to/zig-0.14.1 "$FIRMWAVE_ROOT/scripts/build_bundle.sh"
python3 "$FIRMWAVE_ROOT/scripts/test_firmware.py" \
  --cache-dir "$FIRMWAVE_CACHE" --json

# Dry-run: extract and print the network/monitor-disabled QEMU command.
python3 "$FIRMWAVE_ROOT/scripts/qemu_boot.py" --artifact p210-sd-boot \
  --cache-dir "$FIRMWAVE_CACHE"

# Execution is opt-in and requires qemu-system-arm.
python3 "$FIRMWAVE_ROOT/scripts/qemu_boot.py" --artifact p210-sd-boot \
  --cache-dir "$FIRMWAVE_CACHE" --run
```
...
sources using [Firmwave's prepared email](https://github.com/PhysicistJohn/Atom-NeptuneSDR_Firmwave/blob/main/scripts/request_vendor_materials.md)
...
- `deps/firmwave.lock.json`, [`scripts/resolve_firmwave.py`](scripts/resolve_firmwave.py): immutable companion-repository identity and fail-closed sibling/environment/managed-cache resolver.
- [`Atom-NeptuneSDR_Firmwave`](https://github.com/PhysicistJohn/Atom-NeptuneSDR_Firmwave): ARM source, input locks, artifact audit/build, non-flashable rootfs composition, canonical interface/FFT ABI, and firmware tests.
...
- [Firmwave `docs/P210_FFT_ABI.md`](https://github.com/PhysicistJohn/Atom-NeptuneSDR_Firmwave/blob/main/docs/P210_FFT_ABI.md): canonical ARM-visible FFT register and DMA contract.
...
- [Atom-NeptuneSDR_Firmwave](https://github.com/PhysicistJohn/Atom-NeptuneSDR_Firmwave): audited P210 firmware inputs, ARM runtime, canonical ABI, and non-flashable QEMU-development composition.
