...
FIRMWARE_ROOT=$(python3 "$ATOM_NEPTUNE_ROOT/scripts/resolve_firmware.py")
export NEPTUNESDR_FIRMWARE_ROOT="$FIRMWARE_ROOT"
export PYTHONPATH="$FIRMWARE_ROOT/src:$ATOM_NEPTUNE_ROOT/src${PYTHONPATH:+:$PYTHONPATH}"
OUTPUT=${P210_SOURCE_GATE_OUTPUT:-"$ATOM_NEPTUNE_ROOT/.cache/source-gate"}
mkdir -p "$OUTPUT"
rm -f "$OUTPUT/reference-tests.json" "$OUTPUT/firmware-tests.json" \
...
python3 -m compileall -q src scripts tests cosim/tests \
  "$FIRMWARE_ROOT/src" "$FIRMWARE_ROOT/scripts" "$FIRMWARE_ROOT/tests"
