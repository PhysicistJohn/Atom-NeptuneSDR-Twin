...
python3 "$ROOT/scripts/acceptance_gate.py" start \
    --root "$ROOT" --firmware-root "$FIRMWARE_ROOT" \
...
python3 -m compileall -q "$ROOT/src" "$ROOT/scripts" "$ROOT/tests" \
    "$ROOT/cosim/tests" "$FIRMWARE_ROOT/src" "$FIRMWARE_ROOT/scripts" \
    "$FIRMWARE_ROOT/tests"
python3 "$ROOT/scripts/acceptance_gate.py" test-suite \
    ...
    --require-test test_acceptance_gate.AcceptanceGateTests.test_full_acceptance_requires_clean_twin_and_firmware \
    ...
    --require-test test_firmware_bundle.FirmwareBundleTests.test_hashed_decoy_cannot_substitute_for_canonical_boot_kernel \
    --require-test test_firmware_bundle.FirmwareBundleTests.test_valid_bundle_binds_source_interface_and_every_artifact \
    --require-test test_firmware_dependency.FirmwareDependencyTests.test_explicit_checkout_resolves_exact_release_identity \
    --require-test test_firmware_dependency.FirmwareDependencyTests.test_lock_profile_is_required_and_exact \
    --require-test test_firmware_dependency.FirmwareDependencyTests.test_managed_cache_rejects_symlinked_parent_without_touching_target \
    --require-test test_firmware_dependency.FirmwareDependencyTests.test_skip_worktree_cannot_hide_a_modified_tracked_file \
...
python3 "$ROOT/scripts/acceptance_gate.py" test-suite \
    --start-dir "$FIRMWARE_ROOT/tests" --label acceptance-firmware \
    --summary "$OUTPUT/firmware-tests.json" \
    --log "$OUTPUT/firmware-tests.log" \
...
if [ "$FIRMWARE" = yes ]; then
    python3 "$ROOT/scripts/acceptance_gate.py" test-suite \
        ...
        --require-test test_qemu_fft_source.P210FFTSourceTests.test_qemu_header_refines_the_canonical_firmware_interface \
...
for script in "$FIRMWARE_ROOT"/scripts/*.sh; do
    bash -n "$script"
done
...
python3 -m neptunesdr_firmware fetch p210-system-xsa \
...
if command -v git >/dev/null 2>&1 && git -C "$ROOT" rev-parse --git-dir >/dev/null 2>&1; then
    ...
    git -C "$FIRMWARE_ROOT" diff --check
fi

if [ "$FIRMWARE" = yes ]; then
    python3 "$ROOT/scripts/acceptance_gate.py" finish-full \
        --root "$ROOT" --firmware-root "$FIRMWARE_ROOT" --output "$OUTPUT" \
        --reference-summary "$OUTPUT/reference-tests.json" \
        --firmware-summary "$OUTPUT/firmware-tests.json" \
