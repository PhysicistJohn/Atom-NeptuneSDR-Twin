...
| `deps/firmware.lock.json` | Public Firmware repository URL, full commit/tree identity, and canonical interface SHA-256 | Correctness of Firmware, seller authorship, or hardware compatibility |
| [`Atom-NeptuneSDR-Firmware`](https://github.com/PhysicistJohn/Atom-NeptuneSDR-Firmware) input/runtime locks and manifest | URL, size and SHA-256 for pinned public inputs; audited structure; exact source identity; hashes for every emitted `qemu-development` artifact | Vendor authorship, authenticity, freedom from defects, safe flashability, or compatibility with this revision |
...
| P210-enabled QEMU machine and Firmware ARM FFT runtime | E3 integration bound to both clean repository revisions, Firmware's canonical interface/runtime manifests, the public P210 kernel/DT, official Pluto v0.39 rootfs/`iiod`, real ADI Linux drivers, emulated AD9361/CF-AXI/four-entry DMAC/GEM, nonzero 2x2 IIO block capture, CPU copy to reserved DDR, and proposed 65,536-point FFT/NSFT path | Sustained/lossless 61.44-MSPS processing, zero-copy/direct-PL streaming, vendor-shipped rootfs identity, the public bitstream containing the proposed FFT, synthesized XC7Z020 timing/resources, safe flashability, or physical RF/clock/power |
| Continuous PL spectrum runtime | E2 execution of consecutive 2x2 blocks through 65,536-point FFT/averaging/paired NSFT, live rate/LO epochs, retune loss markers, bounded backpressure and explicit wall-clock lag | Synthesized RTL, post-route timing, or the block existing in the purchased bitstream |
| USB/IP composite appliance | E2 protocol execution of the byte-locked EP0 personality, native-IIO bulk sessions/guest bridge, read-only mass storage, CDC, and RNDIS DHCP/ARP/ICMP/TCP-IIOD contacts through USB/IP | A Linux VHCI attach on this macOS development host, electrical PHY behavior, or descriptor identity of the as-yet uncaptured purchased revision |
...
- [ADI Pluto firmware repository](https://github.com/analogdevicesinc/plutosdr-fw/tree/v0.39): source/build topology and v0.39 release inputs. Firmware's input lock records the release archive digest.
...
- [Pinned public P210 SD boot artifact](https://github.com/wucke13/Neptune-SDR-nix-utils/tree/ba4a958333a3dfec5e5102e30fe017508e1fd6f6): third-party artifact referenced by Firmware's input lock. Its SHA-256 protects the bytes used by tests.
...
- A full E3 acceptance result binds the Twin and Firmware commit/tree/source-state
  identities plus the canonical interface and runtime artifact hashes. A pass
  cannot be transferred to another checkout merely because filenames match.
