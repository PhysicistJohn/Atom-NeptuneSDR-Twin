// Shim: drive the Verilated p210_fft_synth RTL through the rtl_block ABI.
// This is the adapter between the twin's RTL co-processor device and your real
// Verilog. To host your OWN block, copy this file and change exactly two things:
//   1. DUT_CLASS / the #include below -> your Verilated class (Vyour_block).
//   2. RTL_LOG2N -> log2 of your block's size (or pass -DRTL_LOG2N=k at build).
// Everything else (the load/start/done/read tick cadence) carries over for any
// block that keeps the p210_fft_synth port contract. SPDX-License-Identifier: MIT
#include "rtl_block.h"
#include "Vp210_fft_synth.h"
#include "verilated.h"

/* The Verilated top the shim drives. Point these at your own block's class. */
#define DUT_CLASS Vp210_fft_synth
...
{
    (void)log2n;                 /* p210_fft_synth is a fixed-size block */
...
{
    b->dut->io_addr = idx & RTL_ADDR_MASK;
    tick(b); tick(b);            /* addr register + synchronous BRAM read */
